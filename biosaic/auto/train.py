...
class MultiDatasetTrainer:
  ...
  def __init__(self, config: VQConfig, dataset_paths: List[str], dataset_names: List[str], save_dir: str = "/content/drive/MyDrive/checkpoints", log_dir: str = "/content/drive/MyDrive/logs", kmer_size: int = 4, device: str = "cuda" if torch.cuda.is_available() else "cpu"):
    self.config = config
    ...
    # Initialize datasets - must be done before initializing model
    self.datasets = {}
    self.current_dataset_idx = 0
    self.load_datasets()

    # Get the vocab size from one of the loaded datasets
    # Assumes all datasets use the same kmer size and therefore have the same vocab size
    if not self.datasets:
      raise ValueError("No datasets loaded successfully to determine vocab size.")
    first_dataset_name = list(self.datasets.keys())[0]
    vocab_size = bio.Tokenizer("dna", 4, True)._tokenizer.vocab_size
    # Initialize model with the correct vocab size
    self.model = DNA_VQVAE(config, vocab_size).to(device)
    self.n_params = sum(p.numel() for p in self.model.parameters()) / 1e6
    print(f"Model initialized with {self.n_params}M parameters and vocab size {vocab_size}")
    self.logger.info(f"Model initialized with {self.n_params}M parameters and vocab size {vocab_size}")
    ...
    # multi-cast training support
    self.ctx = torch.amp.autocast(device_type='cuda', dtype=torch.bfloat16) if self.device == 'cuda' else nullcontext()
  ...
  def load_datasets(self):
    ...
    if not self.datasets:
      # Changed from raising ValueError here to allow trainer initialization
      # The check is now done after load_datasets returns
  ...
  def compute_loss(self, x: torch.Tensor, x_recon: torch.Tensor, vq_loss: torch.Tensor) -> Dict[str, torch.Tensor]:
    ...
    # Reshape to (B*L, vocab_size) and (B*L,) for cross_entropy
  ...
  def train_step(self, batch_size: int = 32, block_size: int = 512) -> Dict[str, float]:
    ...
    try:
      ...
      self.logger.warning(f"Failed to get batch from {self.dataset_names[self.current_dataset_idx]}, switching dataset: {e}")
      self.switch_dataset()
      dataset = self.get_current_dataset()
      try:
        x, _ = dataset.get_batch("train", batch_size, block_size, self.device)
      except Exception as e_new:
        self.logger.error(f"Failed to get batch from switched dataset {self.dataset_names[self.current_dataset_idx]}: {e_new}")
        raise RuntimeError("Failed to get training batch from any dataset.") from e_new
  ...
  def train(self, num_epochs: int = 100, batch_size: int = 32, block_size: int = 512, eval_interval: int = 500, save_interval: int = 1000, patience: int = 10, learning_rate: float = 1e-4, resume_from: Optional[str] = None):
    """Main training loop"""
    ...
    try:
      ...
      for epoch in range(self.epoch, num_epochs):
        ...
        for step in range(steps_per_epoch):
          ...
          if step % 20 == 0:
            ...
            print(f"Step: {self.global_step} | Dataset: {current_dataset} | Total Loss: {losses['total_loss']} | Recon Loss: {losses['recon_loss']} | VQ Loss: {losses['vq_loss']}")
      ...
      self.emergency_save()
      raise # Re-raise the exception after emergency save
    finally:
      # Final save
      ...
      summary = {
        'params:' : f"{self.n_params} Million",
...
def main():
  ...
  trainer.train(
    ...
    block_size=1024,  # Adjust based on GPU memory
    eval_interval=50,
    save_interval=500,
    patience=1000,
    learning_rate=2e-4
